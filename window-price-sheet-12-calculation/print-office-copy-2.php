...
while ($stmt_1->fetch()) {
    ...
    if ($stmt_3->num_rows()) {
        ...
        while ($stmt_3->fetch()) {
            ...
            $qty_td = $qty_select === 1 && $qty_print[0] === '1' ? '<td style="text-align: center; border: 0.5px solid #000000;">' . $window_price_sheet_12_calculation_quote_item_qty . '</td>' : '';
            $total_td = $qty_select === 1 && $total_print[0] === '1' ? '<td style="text-align: right; border: 0.5px solid #000000;">' . number_format($window_price_sheet_12_calculation_quote_item_total, 2) . '</td>' : '';
