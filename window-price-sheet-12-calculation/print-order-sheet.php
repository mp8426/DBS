...
$query_1 = "SELECT name, locations, materials_and_colours, accessories, per_meters, fitting_charges FROM window_price_sheet_12_calculations WHERE code = ? ORDER BY position ASC";

$stmt_1 = $mysqli->prepare($query_1);
$stmt_1->bind_param('s', $window_price_sheet_12_calculation_code);
$stmt_1->execute();
$stmt_1->bind_result($window_price_sheet_12_calculation_name, $locations_select, $materials_and_colours_select, $accessories_select, $per_meters_select, $fitting_charges_select);
$stmt_1->store_result();
...
while ($stmt_1->fetch()) {
    ...
    $qty_th = $qty_print[2] === '1' ? '<th style="text-align: center; border: 0.5px solid #000000;">Qty</th>' : '';
    ...
    $query_2 = "SELECT code, name, side FROM window_price_sheet_12_calculation_fields WHERE order_sheet = 1 AND window_price_sheet_12_calculation_code = ? ORDER BY position ASC";
    ...
    if ($stmt_3->num_rows()) {
        ...
        $query_3_1_x = "SELECT SUM(price) "
            ...
            . "WHERE "
            . "window_price_sheet_12_calculation_code = ? AND "
            . "cid = ?";

        $stmt_3_1_x = $mysqli->prepare($query_3_1_x);
        $stmt_3_1_x->bind_param('ss', $window_price_sheet_12_calculation_code, $cid);
        $stmt_3_1_x->execute();
        $stmt_3_1_x->bind_result($window_price_sheet_12_calculation_field_price_total);
        $stmt_3_1_x->fetch();
        $stmt_3_1_x->close();

        // $window_price_sheet_12_calculation_quote_item_price_sub_total += $window_price_sheet_12_calculation_field_price_total;
        ...
        while ($stmt_3->fetch()) {
            ...
            //     . "WHERE "
            //     . "window_price_sheet_12_calculation_fields.order_sheet = 1 AND "
            ...
            // $stmt_3_1->execute();
            // $stmt_3_1->bind_result($window_price_sheet_12_calculation_quote_item_field_name, $sub_options, $window_price_sheet_12_calculation_field_side);
            ...
            while ($stmt_3_1->fetch()) {
                ...
                if (empty($field_text)) {
                    $field_text = $window_price_sheet_12_calculation_quote_item_field_name;
                } else {
                    $field_text = $window_price_sheet_12_calculation_quote_item_field_name . '<br>>> ' . $field_text;
                }
            ...
            $location_td = $locations_select === 1 && $location_print[2] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $window_price_sheet_12_calculation_quote_item_location)[0] . '</td>' : '';
            $width_x_td = $width_x_print[2] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_price_sheet_12_calculation_quote_item_width . '</td>' : '';
            $drop_x_td = $drop_x_print[2] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_price_sheet_12_calculation_quote_item_drop . '</td>' : '';
            $type_td = $type_print[2] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $window_price_sheet_12_calculation_quote_item_type)[0] . '</td>' : '';
            $material_td = $materials_and_colours_select === 1 && $material_print[2] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $window_price_sheet_12_calculation_quote_item_material)[0] . '</td>' : '';
            $colour_td = $materials_and_colours_select === 1 && $colour_print[2] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $window_price_sheet_12_calculation_quote_item_colour)[0] . '</td>' : '';
            $price_td = $price_print[2] === '1' ? '<td style="text-align: right; border: 0.5px solid #000000;">' . number_format($window_price_sheet_12_calculation_quote_item_price, 2) . '</td>' : '';
            $qty_td = $qty_print[2] === '1' ? '<td style="text-align: center; border: 0.5px solid #000000;">' . $window_price_sheet_12_calculation_quote_item_qty . '</td>' : '';
            $total_td = $total_print[2] === '1' ? '<td style="text-align: right; border: 0.5px solid #000000;">' . number_format($window_price_sheet_12_calculation_quote_item_total, 2) . '</td>' : '';

            $price_td_num_rows = $price_print[2] === '1' ? 1 : 0;
            $qty_td_num_rows = $qty_print[2] === '1' ? 1 : 0;
            ...
            if ($window_price_sheet_12_calculation_quote_item_notes || $window_price_sheet_12_calculation_quote_item_accessories || $window_price_sheet_12_calculation_quote_item_per_meters || $window_price_sheet_12_calculation_quote_item_fitting_charges) {
                ...
                if ($window_price_sheet_12_calculation_quote_item_notes && $note_print[2] === '1') {
                ...
                if ($window_price_sheet_12_calculation_quote_item_accessories && $accessories_select === 1 && $accessory_print[2] === '1') {
                ...
                if ($window_price_sheet_12_calculation_quote_item_per_meters && $per_meters_select === 1 && $per_meter_print[2] === '1') {
                ...
                if ($window_price_sheet_12_calculation_quote_item_fitting_charges && $fitting_charges_select === 1 && $fitting_charge_print[2] === '1') {
                ...
                if (
                    $accessories_select === 1 && $accessory_print[2] === '1' && $window_price_sheet_12_calculation_quote_item_accessories ||
                    $per_meters_select === 1 && $per_meter_print[2] === '1' && $window_price_sheet_12_calculation_quote_item_per_meters ||
                    $fitting_charges_select === 1 && $fitting_charge_print[2] === '1' && $window_price_sheet_12_calculation_quote_item_fitting_charges
                ) {
