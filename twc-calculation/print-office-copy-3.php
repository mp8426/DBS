...
while ($stmt_1->fetch()) {
    ...
    $query_2 = "SELECT code, name, side FROM twc_calculation_fields WHERE office_copy = 1 AND twc_calculation_code = ? ORDER BY position ASC";
    ...
    if ($stmt_3->num_rows()) {
        ...
        while ($stmt_3->fetch()) {
            ...
            while ($stmt_3_1->fetch()) {
                ...
                if ($twc_calculation_field_side === 0) { // Right Side
                    $field_td_right .= '<td style="border: 0.5px solid #787877;">' . $twc_calculation_quote_item_field_name . $twc_calculation_quote_item_field_price_x . '</td>';
                }
                if ($twc_calculation_field_side === 1) { // Left Side
                    $field_td_left .= '<td style="border: 0.5px solid #787877;">' . $twc_calculation_quote_item_field_name . $twc_calculation_quote_item_field_price_x . '</td>';
                }
            }
            $stmt_3_1->close();
            ...
            $location_td = $locations_select === 1 && $location_print[0] === '1' ? '<td style="border: 0.5px solid #787877;">' . explode('<->', $twc_calculation_quote_item_location)[0] . '</td>' : '';
            $width_x_td = $width_x_print[0] === '1' ? '<td style="border: 0.5px solid #787877;">' . $twc_calculation_quote_item_width . '</td>' : '';
            $drop_x_td = $drop_x_print[0] === '1' ? '<td style="border: 0.5px solid #787877;">' . $twc_calculation_quote_item_drop . '</td>' : '';
            $type_td = $type_print[0] === '1' ? '<td style="border: 0.5px solid #787877;">' . explode('<->', $twc_calculation_quote_item_type)[0] . '</td>' : '';
            $material_td = $materials_and_colours_select === 1 && $material_print[0] === '1' ? '<td style="border: 0.5px solid #787877;">' . explode('<->', $twc_calculation_quote_item_material)[0] . '</td>' : '';
            $colour_td = $materials_and_colours_select === 1 && $colour_print[0] === '1' ? '<td style="border: 0.5px solid #787877;">' . explode('<->', $twc_calculation_quote_item_colour)[0] . '</td>' : '';
            $price_td = $price_print[0] === '1' ? '<td style="text-align: right; border: 0.5px solid #787877;">' . number_format($twc_calculation_quote_item_price, 2) . '</td>' : '';
