...
$query_1 = "SELECT name, locations, materials_and_colours, accessories, per_meters, fitting_charges FROM twc_calculations WHERE code = ? ORDER BY position ASC";

$stmt_1 = $mysqli->prepare($query_1);
$stmt_1->bind_param('s', $twc_calculation_code);
$stmt_1->execute();
$stmt_1->bind_result($twc_calculation_name, $locations_select, $materials_and_colours_select, $accessories_select, $per_meters_select, $fitting_charges_select);
$stmt_1->store_result();
...
while ($stmt_1->fetch()) {
    ...
    $query_2 = "SELECT code, name, side FROM twc_calculation_fields WHERE order_sheet = 1 AND twc_calculation_code = ? ORDER BY position ASC";
    ...
    if ($stmt_3->num_rows()) {
        ...
        $query_3_1_x = "SELECT SUM(price) "
                ...
                . "WHERE "
                . "twc_calculation_code = ? AND "
                . "cid = ?";

        $stmt_3_1_x = $mysqli->prepare($query_3_1_x);
        $stmt_3_1_x->bind_param('ss', $twc_calculation_code, $cid);
        $stmt_3_1_x->execute();
        $stmt_3_1_x->bind_result($twc_calculation_field_price_total);
        $stmt_3_1_x->fetch();
        $stmt_3_1_x->close();

        $twc_calculation_quote_item_price_sub_total += $twc_calculation_field_price_total;

        while ($stmt_3->fetch()) {

            $csv_price_sheet = __DIR__ . '/../cPanel/price-sheets/' . explode('<->', $twc_calculation_quote_item_type)[1] . '-' . $twc_calculation_code . '.csv';
            $twc_calculation_quote_item_price = get_price_from_csv($csv_price_sheet, $twc_calculation_quote_item_width, $twc_calculation_quote_item_drop);
            ...
            $query_3_1 = "SELECT twc_calculation_quote_item_fields.name, "
                    ...
                    . "WHERE "
                    . "twc_calculation_fields.order_sheet = 1 AND "
            ...
            while ($stmt_3_1->fetch()) {
                if ($twc_calculation_field_side === 0) { // Right Side
                    $field_td_right .= '<td style="border: 0.5px solid #000000;">' . $twc_calculation_quote_item_field_name . '</td>';
                }
                if ($twc_calculation_field_side === 1) { // Left Side
                    $field_td_left .= '<td style="border: 0.5px solid #000000;">' . $twc_calculation_quote_item_field_name . '</td>';
                }
            }
            $stmt_3_1->close();
            ...
            $location_td = $locations_select === 1 && $location_print[2] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $twc_calculation_quote_item_location)[0] . '</td>' : '';
            $width_x_td = $width_x_print[2] === '1' ? '<td style="border: 0.5px solid #000000;">' . $twc_calculation_quote_item_width . '</td>' : '';
            $drop_x_td = $drop_x_print[2] === '1' ? '<td style="border: 0.5px solid #000000;">' . $twc_calculation_quote_item_drop . '</td>' : '';
            $type_td = $type_print[2] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $twc_calculation_quote_item_type)[0] . '</td>' : '';
            $material_td = $materials_and_colours_select === 1 && $material_print[2] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $twc_calculation_quote_item_material)[0] . '</td>' : '';
            $colour_td = $materials_and_colours_select === 1 && $colour_print[2] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $twc_calculation_quote_item_colour)[0] . '</td>' : '';
            $price_td = $price_print[2] === '1' ? '<td style="text-align: right; border: 0.5px solid #000000;">' . number_format($twc_calculation_quote_item_price, 2) . '</td>' : '';
            ...
            if ($twc_calculation_quote_item_notes || $twc_calculation_quote_item_accessories || $twc_calculation_quote_item_per_meters || $twc_calculation_quote_item_fitting_charges) {
                ...
                if ($twc_calculation_quote_item_notes && $note_print[2] === '1') {
                ...
                if ($twc_calculation_quote_item_accessories && $accessories_select === 1 && $accessory_print[2] === '1') {
                ...
                if ($twc_calculation_quote_item_per_meters && $per_meters_select === 1 && $per_meter_print[2] === '1') {
                ...
                if ($twc_calculation_quote_item_fitting_charges && $fitting_charges_select === 1 && $fitting_charge_print[2] === '1') {
                ...
                if ($accessories_select === 1 && $accessory_print[2] === '1' && $twc_calculation_quote_item_accessories ||
                        $per_meters_select === 1 && $per_meter_print[2] === '1' && $twc_calculation_quote_item_per_meters ||
                        $fitting_charges_select === 1 && $fitting_charge_print[2] === '1' && $twc_calculation_quote_item_fitting_charges
                ) {
