...
while ($stmt_1->fetch()) {
    ...
    if ($stmt_3->num_rows()) {
        ...
        while ($stmt_3->fetch()) {
            ...
            $price_sub_total += $twc_calculation_quote_item_price;
...
//                }
            ...
            while ($stmt_3_2->fetch()) {
                ...
                $quote_item_accessories_price_td = $price_td_num_rows ? '<td style="border: 0.5px solid #000000; text-align: right;">' . number_format($twc_calculation_quote_item_accessory_total, 2) . '</td>' : '';

                $quote_item_accessories .= '<tr>'
                        . '<td style="border: 0.5px solid #000000; text-align: center;">' . $quote_item_no . '</td>'
                        . '<td style="border: 0.5px solid #000000;" colspan="' . $table_colspan . '">' . $twc_calculation_quote_item_accessory_name . ' ($' . $twc_calculation_quote_item_accessory_price . ' x ' . $twc_calculation_quote_item_accessory_qty . ')</td>'
            ...
            while ($stmt_3_3->fetch()) {
                ...
                $quote_item_per_meters_price_td = $price_td_num_rows ? '<td style="border: 0.5px solid #000000; text-align: right;">' . number_format($twc_calculation_quote_item_per_meter_total, 2) . '</td>' : '';

                $quote_item_per_meters .= '<tr>'
                        . '<td style="border: 0.5px solid #000000; text-align: center;">' . $quote_item_no . '</td>'
                        . '<td style="border: 0.5px solid #000000;" colspan="' . $table_colspan . '">' . $twc_calculation_quote_item_per_meter_name . ' ($' . $twc_calculation_quote_item_per_meter_price . ' x ' . $twc_calculation_quote_item_per_meter_width . ')</td>'
            ...
            while ($stmt_3_4->fetch()) {
                ...
                $quote_item_fitting_charges_price_td = $price_td_num_rows ? '<td style="border: 0.5px solid #000000; text-align: right;">' . number_format($twc_calculation_quote_item_fitting_charge_price, 2) . '</td>' : '';

                $quote_item_fitting_charges .= '<tr>'
                        . '<td style="border: 0.5px solid #000000; text-align: center;">' . $quote_item_no . '</td>'
                        . '<td style="border: 0.5px solid #000000;" colspan="' . $table_colspan . '">' . $twc_calculation_quote_item_fitting_charge_name . '</td>'
                        . $quote_item_fitting_charges_price_td
                        . '</tr>';
            }
            $stmt_3_4->close();

            $location_td = $locations_select === 1 && $location_print[0] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $twc_calculation_quote_item_location)[0] . '</td>' : '';
            $width_x_td = $width_x_print[0] === '1' ? '<td style="border: 0.5px solid #000000;">' . $twc_calculation_quote_item_width . '</td>' : '';
            $drop_x_td = $drop_x_print[0] === '1' ? '<td style="border: 0.5px solid #000000;">' . $twc_calculation_quote_item_drop . '</td>' : '';
            $type_td = $type_print[0] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $twc_calculation_quote_item_type)[0] . '</td>' : '';
            $material_td = $materials_and_colours_select === 1 && $material_print[0] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $twc_calculation_quote_item_material)[0] . '</td>' : '';
            $colour_td = $materials_and_colours_select === 1 && $colour_print[0] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $twc_calculation_quote_item_colour)[0] . '</td>' : '';
            $price_td = $price_print[0] === '1' ? '<td style="text-align: right; border: 0.5px solid #000000;">' . number_format($twc_calculation_quote_item_price, 2) . '</td>' : '';
            ...
            if ($group_discount_print[0] === '1') {
                ...
                $twc_calculation_quote_item_discount_value = $price_sub_total * $twc_calculation_quote_item_discount / 100;
                $twc_calculation_quote_item_price_total = $price_sub_total - $twc_calculation_quote_item_discount_value;
                ...
                $twc_calculation_total = '<table cellpadding="4" cellspacing="0" style="text-align: center;" nobr="true">'
                        ...
                        . '</tr>'
                        . '<tr style="font-size: 0.9em;">'
                        . '<th style="border: 0.5px solid #000000; text-align: right; font-weight: bold;" colspan="' . $total_table_colspan . '">Discount (' . $twc_calculation_quote_item_discount . '%) </th>'
        ...
        $twc_calculation_quote_tables .= $twc_calculation_table_header . $quote_items . $table_more . $twc_calculation_total . "<div></div>";
    } else {
        $twc_calculation_quote_tables .= "";
    }
    $stmt_3->close();
