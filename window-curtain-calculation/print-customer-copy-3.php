...
$price_name_list_for_summary = "";

$query_1 = "SELECT code, name, locations, accessories,per_meters, fitting_charges, qty FROM window_curtain_calculations ORDER BY position ASC";

$stmt_1 = $mysqli->prepare($query_1);
$stmt_1->execute();
$stmt_1->bind_result($window_curtain_calculation_code, $window_curtain_calculation_name, $locations_select, $accessories_select, $per_meters_select, $fitting_charges_select, $qty_select);
$stmt_1->store_result();
...
while ($stmt_1->fetch()) {
    ...
    $query_2x = "SELECT location, width, right_return, left_return, overlap, fullness, supplier, fabric, fabric_colour, qty_drop, curtain_type_1, cont_meter, curtain_type_2, drop_x, hem_heading, pattern_repeate, fabric_cut_length, fabric_qty, price, qty, total, note, group_discount, accessory, per_meter, fitting_charge FROM window_curtain_calculation_fixed_fields_visibility WHERE window_curtain_calculation_code = ?";

    $stmt_2x = $mysqli->prepare($query_2x);
    $stmt_2x->bind_param('s', $window_curtain_calculation_code);
    $stmt_2x->execute();
    $stmt_2x->bind_result($location_print, $width_print, $right_return_print, $left_return_print, $overlap_print, $fullness_print, $supplier_print, $fabric_print, $fabric_colour_print, $qty_drop_print, $curtain_type_1_print, $cont_meter_print, $curtain_type_2_print, $drop_print, $hem_heading_print, $pattern_repeate_print, $fabric_cut_length_print, $fabric_qty_print, $price_print, $qty_print, $total_print, $note_print, $group_discount_print, $accessory_print, $per_meter_print, $fitting_charge_print);
    $stmt_2x->fetch();
    $stmt_2x->close();

    $location_th = $locations_select === 1 && $location_print[1] === '1' ? '<th style="border: 0.5px solid #000000;">Location</th>' : '';
    $width_th = $width_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Width</td>' : '';
    $right_return_th = $right_return_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Right Return</td>' : '';
    $left_return_th = $left_return_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Left Return</td>' : '';
    $overlap_th = $overlap_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Overlap</td>' : '';
    $fullness_th = $fullness_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Fullness</td>' : '';
    $supplier_th = $supplier_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Supplier</td>' : '';
    $fabric_th = $fabric_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Fabric</td>' : '';
    $fabric_colour_th = $fabric_colour_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Colour</td>' : '';
    $qty_drop_th = $qty_drop_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Qty Drop</td>' : '';
    $curtain_type_1_th = $curtain_type_1_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Non Continuous</td>' : '';
    $cont_meter_th = $cont_meter_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Cont. Meter</td>' : '';
    $curtain_type_2_th = $curtain_type_2_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Continuous </td>' : '';
    $drop_x_th = $drop_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Drop</td>' : '';
    $hem_heading_th = $hem_heading_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Hem Heading</td>' : '';
    $pattern_repeate_th = $pattern_repeate_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Pattern Repeate</td>' : '';
    $fabric_cut_length_th = $fabric_cut_length_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">Fabric Cut Length</td>' : '';
    ...
    $query_2 = "SELECT code, name, side FROM window_curtain_calculation_fields WHERE customer_copy = 1 AND window_curtain_calculation_code = ? ORDER BY position ASC";
    ...
    $window_curtain_calculation_table_header = '<span nobr="true">'
        . '<h1 style="color:#303030;">' . $window_curtain_calculation_name . '</h1>'
        . '<table cellpadding="4" cellspacing="0" style="text-align: center; background-color: #f1f1f1; height: 18px; line-height:25px;">'
    ...
    if ($stmt_3->num_rows()) {
        ...
        while ($stmt_3->fetch()) {
            ...
            // $window_curtain_calculation_quote_item_price_sub_total += $window_curtain_calculation_quote_item_price;
            $window_curtain_calculation_quote_item_price_sub_total += $window_curtain_calculation_quote_item_total;
            ...
            //     . "window_curtain_calculation_quote_item_fields.sub_options, "
            ...
            //     . "WHERE "
            //     . "window_curtain_calculation_fields.customer_copy = 1 AND "
            ...
            // $stmt_3_1->execute();
            // $stmt_3_1->bind_result($window_curtain_calculation_quote_item_field_name, $window_curtain_calculation_quote_item_field_price, $sub_options, $window_curtain_calculation_field_side);
            ...
            $query_3_1 = "SELECT window_curtain_calculation_quote_item_fields.name, "
                ...
                . "window_curtain_calculation_quote_item_fields.sub_options, "
                . "window_curtain_calculation_fields.side "
                . "FROM window_curtain_calculation_fields "
                . "LEFT JOIN window_curtain_calculation_quote_item_fields ON "
                . "window_curtain_calculation_quote_item_fields.window_curtain_calculation_quote_item_code = ? AND "
                . "window_curtain_calculation_quote_item_fields.window_curtain_calculation_code = ? AND "
                . "window_curtain_calculation_quote_item_fields.cid = ? AND "
                . "window_curtain_calculation_fields.code = window_curtain_calculation_quote_item_fields.window_curtain_calculation_field_code "
                . "WHERE "
                . "window_curtain_calculation_fields.customer_copy = 1 AND "
                . "window_curtain_calculation_fields.window_curtain_calculation_code = ? "
                . "ORDER BY window_curtain_calculation_fields.position ASC";

            $stmt_3_1 = $mysqli->prepare($query_3_1);
            $stmt_3_1->bind_param('ssss', $window_curtain_calculation_quote_item_code, $window_curtain_calculation_code, $cid, $window_curtain_calculation_code);
            $stmt_3_1->execute();
            $stmt_3_1->bind_result($window_curtain_calculation_quote_item_field_name, $window_curtain_calculation_quote_item_field_price, $sub_options, $window_curtain_calculation_field_side);
            $stmt_3_1->store_result();
            $field_td_num_rows = $stmt_3_1->num_rows;
            ...
            while ($stmt_3_1->fetch()) {
                ...
                if (empty($field_text)) {
                    $field_text = $window_curtain_calculation_quote_item_field_name . $window_curtain_calculation_quote_item_field_price_x;
                } else {
                    $field_text = $window_curtain_calculation_quote_item_field_name . $window_curtain_calculation_quote_item_field_price_x . '<br>>> ' . $field_text;
                }
            ...
            $location_td = $locations_select === 1 && $location_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $window_curtain_calculation_quote_item_location)[0] . '</td>' : '';
            $width_td = $width_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_curtain_calculation_quote_item_width . '</td>' : '';
            $right_return_td = $right_return_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_curtain_calculation_quote_item_right_return . '</td>' : '';
            $left_return_td = $left_return_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_curtain_calculation_quote_item_left_return . '</td>' : '';
            $overlap_td = $overlap_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_curtain_calculation_quote_item_overlap . '</td>' : '';
            $fullness_td = $fullness_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_curtain_calculation_quote_item_fullness . '</td>' : '';
            $supplier_td = $supplier_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $window_curtain_calculation_quote_item_supplier)[0] . '</td>' : '';
            $fabric_td = $fabric_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $window_curtain_calculation_quote_item_fabric)[0] . '</td>' : '';
            $fabric_colour_td = $fabric_colour_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $window_curtain_calculation_quote_item_fabric_colour)[0] . '</td>' : '';
            $qty_drop_td = $qty_drop_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_curtain_calculation_quote_item_qty_drop . '</td>' : '';
            $curtain_type_1_td = $curtain_type_1_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $window_curtain_calculation_quote_item_curtain_type_1)[0] . '</td>' : '';
            $cont_meter_td = $cont_meter_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_curtain_calculation_quote_item_continuous_meter . '</td>' : '';
            $curtain_type_2_td = $curtain_type_2_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $window_curtain_calculation_quote_item_curtain_type_2)[0] . '</td>' : '';
            $drop_x_td = $drop_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_curtain_calculation_quote_item_drop . '</td>' : '';
            $hem_heading_td = $hem_heading_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_curtain_calculation_quote_item_hem_heading . '</td>' : '';
            $pattern_repeate_td = $pattern_repeate_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_curtain_calculation_quote_item_pattern_repeate . '</td>' : '';
            $fabric_cut_length_td = $fabric_cut_length_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_curtain_calculation_quote_item_fabric_cut_length . '</td>' : '';
            $fabric_qty_td = $fabric_qty_print[1] === '1' ? '<td style="border: 0.5px solid #000000;">' . $window_curtain_calculation_quote_item_fabric_qty . '</td>' : '';
            $price_td = $price_print[1] === '1' ? '<td style="text-align: right; border: 0.5px solid #000000;">' . number_format($window_curtain_calculation_quote_item_price, 2) . '</td>' : '';
            $qty_td = $qty_select === 1 && $qty_print[1] === '1' ? '<td style="text-align: center; border: 0.5px solid #000000;">' . $window_curtain_calculation_quote_item_qty . '</td>' : '';
            $total_td = $qty_select === 1 && $price_print[1] === '1' ? '<td style="text-align: right; border: 0.5px solid #000000;">' . number_format($window_curtain_calculation_quote_item_total, 2) . '</td>' : '';
            ...
            if ($window_curtain_calculation_quote_item_notes || $window_curtain_calculation_quote_item_accessories || $window_curtain_calculation_quote_item_per_meters || $window_curtain_calculation_quote_item_fitting_charges) {
                ...
                if ($window_curtain_calculation_quote_item_notes && $note_print[1] === '1') {
                ...
                if ($window_curtain_calculation_quote_item_accessories && $accessories_select === 1 && $accessory_print[1] === '1') {
                ...
                if ($window_curtain_calculation_quote_item_per_meters && $per_meters_select === 1 && $per_meter_print[1] === '1') {
                ...
                if ($window_curtain_calculation_quote_item_fitting_charges && $fitting_charges_select === 1 && $fitting_charge_print[1] === '1') {
                ...
                if (
                    $accessories_select === 1 && $accessory_print[1] === '1' && $accessory_print[1] === '1' && $window_curtain_calculation_quote_item_accessories ||
                    $per_meters_select === 1 && $per_meter_print[1] === '1' && $window_curtain_calculation_quote_item_per_meters ||
                    $fitting_charges_select === 1 && $fitting_charge_print[1] === '1' && $window_curtain_calculation_quote_item_fitting_charges
                ) {
            ...
            if ($group_discount_print[1] === '1') {
                ...
                $window_curtain_calculation_total_table_colspan = $table_td_num_rows + $field_td_num_rows + $price_td_num_rows + $qty_td_num_rows + $total_td_num_rows;
                $window_curtain_calculation_total = '<table cellpadding="4" cellspacing="0" style="text-align: center;" nobr="true">'
                    . '<tr style="font-size: 0.9em;">'
                    . '<th style="width:804px; border-top: 0.5px solid #616060; border-left: 0.5px solid #616060; border-bottom: 0.5px solid #616060; color:#303030; font-size: 1.9em; text-align: center; font-weight: 14px; vertical-align: middle;" rowspan="3">'.$price_sheet_cal_name_for_summary.'</th>'
                    . '<th style="width:100px; border-top: 0.5px solid #616060; border-right: 0.5px solid #616060; color:#303030; text-align: right; font-weight: bold;" colspan="' . $window_curtain_calculation_total_table_colspan . '">Sub Total </th>'
                    . '<th style="width:100px; border: 0.5px solid #000000; text-align: right; font-weight: bold;">' . number_format($window_curtain_calculation_quote_item_price_sub_total, 2) . '</th>'
                    . '</tr>'
                    . '<tr style="font-size: 0.9em;">'
                    . '<th style="border-top: 0.5px solid #616060; border-right: 0.5px solid #616060; border-bottom: 0.5px solid #616060; color:#277cbe; text-align: right; font-weight: bold;" colspan="' . $window_curtain_calculation_total_table_colspan . '">Discount (' . $window_curtain_calculation_quote_item_discount . '%) </th>'
                    . '<th style="border: 0.5px solid #616060;  color:#277cbe; text-align: right; font-weight: bold;">-' . number_format($window_curtain_calculation_quote_item_discount_value, 2) . '</th>'
                    . '</tr>'
                    // . '<tr style="font-size: 0.9em;">'
        ...
        $price_list[] = array(
            'p_name' => $window_curtain_calculation_name,
            'quantity' => $row_no,
            'price' => number_format($window_curtain_calculation_quote_item_price_sub_total, 2)
        );
