...
while ($stmt_1->fetch()) {
    ...
    $field_th_left = '';
    $cost_th = $cost_select === 1 && $cost_print[0] === '1' ? '<td style="border: 0.5px solid #000000;">Cost</td>' : '';
    $markup_th = $markup_select === 1 && $markup_print[0] === '1' ? '<td style="border: 0.5px solid #000000;">Markup</td>' : '';
    ...
    $cost_td_num_rows = $cost_select === 1 && $cost_print[0] === '1' ? 1 : 0;
    $markup_td_num_rows = $markup_select === 1 && $markup_print[0] === '1' ? 1 : 0;
    ...
    $query_2 = "SELECT code, name, side FROM form_no_1_calculation_fields WHERE office_copy = 1 AND form_no_1_calculation_code = ? ORDER BY position ASC";
    ...
    if ($stmt_3->num_rows()) {
        ...
        while ($stmt_3->fetch()) {
            ...
            // $price_sub_total += $form_no_1_calculation_quote_item_price;
            $price_sub_total += $form_no_1_calculation_quote_item_total;
            ...
            //     . "WHERE "
            //     . "form_no_1_calculation_fields.office_copy = 1 AND "
            ...
            $table_colspan = $field_td_num_rows + $qty_td_num_rows + $total_td_num_rows;
            ...
            while ($stmt_3_2->fetch()) {
                ...
                $quote_item_accessories_price_td = $price_td_num_rows ? '<td style="border: 0.5px solid #000000; text-align: right;">' . number_format($form_no_1_calculation_quote_item_accessory_total, 2) . '</td>' : '';

                $quote_item_accessories .= '<tr>'
                    . '<td style="border: 0.5px solid #000000; text-align: center;">' . $row_no . '</td>'
                    . '<td style="border: 0.5px solid #000000;" colspan="' . $table_colspan . '">' . $form_no_1_calculation_quote_item_accessory_name . ' ($' . $form_no_1_calculation_quote_item_accessory_price . ' x ' . $form_no_1_calculation_quote_item_accessory_qty . ')</td>'
            ...
            while ($stmt_3_3->fetch()) {
                ...
                $quote_item_per_meters_price_td = $price_td_num_rows ? '<td style="border: 0.5px solid #000000; text-align: right;">' . number_format($form_no_1_calculation_quote_item_per_meter_total, 2) . '</td>' : '';

                $quote_item_per_meters .= '<tr>'
                    . '<td style="border: 0.5px solid #000000; text-align: center;">' . $row_no . '</td>'
                    . '<td style="border: 0.5px solid #000000;" colspan="' . $table_colspan . '">' . $form_no_1_calculation_quote_item_per_meter_name . ' ($' . $form_no_1_calculation_quote_item_per_meter_price . ' x ' . $form_no_1_calculation_quote_item_per_meter_width . ')</td>'
            ...
            while ($stmt_3_4->fetch()) {
                ...
                $quote_item_fitting_charges_price_td = $price_td_num_rows ? '<td style="border: 0.5px solid #000000; text-align: right;">' . number_format($form_no_1_calculation_quote_item_fitting_charge_price, 2) . '</td>' : '';

                $quote_item_fitting_charges .= '<tr>'
                    . '<td style="border: 0.5px solid #000000; text-align: center;">' . $row_no . '</td>'
                    . '<td style="border: 0.5px solid #000000;" colspan="' . $table_colspan . '">' . $form_no_1_calculation_quote_item_fitting_charge_name . '</td>'
                    . $quote_item_fitting_charges_price_td
                    . '</tr>';
            }
            $stmt_3_4->close();

            $location_td = $locations_select === 1 && $location_print[0] === '1' ? '<td style="border: 0.5px solid #000000;">' . explode('<->', $form_no_1_calculation_quote_item_location)[0] . '</td>' : '';
            $cost_td = $cost_select === 1 && $cost_print[0] === '1' ? '<td style="border: 0.5px solid #000000;">' . $form_no_1_calculation_quote_item_cost . '</td>' : '';
            $markup_td = $markup_select === 1 && $markup_print[0] === '1' ? '<td style="border: 0.5px solid #000000;">' . $form_no_1_calculation_quote_item_markup . '</td>' : '';
            $price_td = $price_print[0] === '1' ? '<td style="text-align: right; border: 0.5px solid #000000;">' . number_format($form_no_1_calculation_quote_item_price, 2) . '</td>' : '';
            $qty_td = $qty_select === 1 && $qty_print[0] === '1' ? '<td style="text-align: center; border: 0.5px solid #000000;">' . $form_no_1_calculation_quote_item_qty . '</td>' : '';
            $total_td = $qty_select === 1 &&  $total_print[0] === '1' ? '<td style="text-align: right; border: 0.5px solid #000000;">' . number_format($form_no_1_calculation_quote_item_total, 2) . '</td>' : '';
            ...
            if ($form_no_1_calculation_quote_item_notes && $note_print[0] === '1') {
            ...
            if ($group_discount_print[0] === '1') {
                ...
                $form_no_1_calculation_quote_item_discount_value = $price_sub_total * $form_no_1_calculation_quote_item_discount / 100;
                $form_no_1_calculation_quote_item_price_total = $price_sub_total - $form_no_1_calculation_quote_item_discount_value;

                $form_no_1_calculation_total = '<table cellpadding="4" cellspacing="0" style="text-align: center;" nobr="true">'
                    . '<tr style="font-size: 0.9em;">'
                    . '<th style="border: 0.5px solid #000000; text-align: right; font-weight: bold;" colspan="' . $total_table_colspan . '"></th>'
                    . '<th style="border: 0.5px solid #000000; text-align: right; font-weight: bold;">' . number_format($price_sub_total, 2) . '</th>'
                    . '</tr>'
                    . '<tr style="font-size: 0.9em;">'
                    . '<th style="border: 0.5px solid #000000; text-align: right; font-weight: bold;" colspan="' . $total_table_colspan . '">Discount (' . $form_no_1_calculation_quote_item_discount . '%) </th>'
        ...
        $form_no_1_calculation_quote_tables .= $form_no_1_calculation_table_header . $quote_items . $table_more . $form_no_1_calculation_total . "<div></div>";
    } else {
        $form_no_1_calculation_quote_tables .= "";
    }
    $stmt_3->close();
